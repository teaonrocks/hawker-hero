...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, "public/images"); // Make sure this folder exists and is accessible
  },
...
router.post(
  ...
  (req, res) => {
    ...
    const sql = `
    INSERT INTO reviews (user_id, stall_id, rating, comment, image_url, created_at)
    VALUES (?, ?, ?, ?, ?, NOW())
  `;

    db.query(
      sql,
      [req.session.user.id, stall_id, rating, comment, imageUrl],
      (err, result) => {
        if (err) {
...
// POST - Edit review
router.post("/editReviews/:id", checkAuthenticated, upload.single("image"), (req, res) => {
  const reviewId = req.params.id;
  const { stall_id, rating, comment } = req.body;
  const isAdmin = req.session.user.role === "admin";
  const userId = req.session.user.id;
  ...
  if (isAdmin) {
    sql = imageUrl
      ? "UPDATE reviews SET stall_id = ?, rating = ?, comment = ?, image_url = ? WHERE id = ?"
      : "UPDATE reviews SET stall_id = ?, rating = ?, comment = ? WHERE id = ?";

    params = imageUrl
      ? [stall_id, rating, comment, imageUrl, reviewId]
      : [stall_id, rating, comment, reviewId];
  } else {
    sql = imageUrl
      ? "UPDATE reviews SET stall_id = ?, rating = ?, comment = ?, image_url = ? WHERE id = ? AND user_id = ?"
      : "UPDATE reviews SET stall_id = ?, rating = ?, comment = ? WHERE id = ? AND user_id = ?";

    params = imageUrl
      ? [stall_id, rating, comment, imageUrl, reviewId, userId]
