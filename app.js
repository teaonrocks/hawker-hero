...
app.post("/favorites/add", checkAuthenticated, (req, res) => {
	...
	const checkSql =
		"SELECT * FROM favorites WHERE user_id = ? AND stall_id = ? AND food_id = ?";
    ...
    `;
		db.query(
			insertSql,
			[user_id, stall_id || null, food_id || null, notes || null],
			(err) => {
				if (err) return res.status(500).send("Database error");
...
app.post(
    ...
    WHERE id = ?`;
		...
		db.query(
			sql,
			[stall_id || null, food_id || null, notes || null, req.params.id],
			(err) => {
				if (err) return res.status(500).send("Database error");
